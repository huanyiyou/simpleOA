...
<!--显示表单内容-->
<div id=MainArea>
    <form:form action="/role/setPrivilege" modelAttribute="role">
        <form:hidden path="id"/>
        <div class="ItemBlock_Title1"><!-- 信息说明 -->
            <div class="ItemBlock_Title1">
                <img border="0" width="4" height="7"
                     src="${pageContext.request.contextPath}/style/blue/images/item_point.gif"/> 正在为【${role.name}】配置权限
            </div>
        </div>
                                ...
                                <c:forEach items="${applicationScope.privilegeTopList}" var="privilege">
                                    <li>
                                        <input type="checkbox" name="privilegeIds" value="${privilege.id}"
                                               id="cb_${privilege.id}"
                                                <c:forEach items="${role.privilegeIds}" var="id">
                                                    <c:if test="${privilege.id == id}">
                                                        checked
                                                    </c:if>
                                                </c:forEach>
                                                />
                                        <label for='cb_${privilege.id}'><span
                                                class='folder'>${privilege.name}</span></label>
                                        <ul>
                                                        ...
                                                        <c:forEach items="${privilege.children}" var="privilege">
                                                            <li>
                                                                <input type="checkbox" name="privilegeIds"
                                                                       value="${privilege.id}" id="cb_${privilege.id}"
                                                                        <c:forEach items="${role.privilegeIds}"
                                                                                   var="id">
                                                                            <c:if test="${privilege.id == id}">
                                                                                checked
                                                                            </c:if>
